...
import Taro, { FC, NodesRef } from '@tarojs/taro';
import { Block, View, Text, Canvas } from '@tarojs/components';
...
const Comp: FC<{
  ...
  let refCVS = useRef<Taro.Canvas>(null);
  let refCTX = useRef<CanvasRenderingContext2D>(null);

  let getCanvas = useCallback(
    (
      select: string,
      opt?: {
        width?: number;
      ...
      return new Promise<{
        ...
        setTimeout(() => {
          ...
          query
            ...
            .exec(([res]) => {
              let { node: cvs, width, height } = res;

              if (cvs) {
                if (opt?.width) cvs.width = opt.width;
                if (opt?.height) cvs.height = opt.height;
  ...
  let drawImage = useCallback(
    (opt: IImage) => {
      return new Promise<Taro.Image>((resolve, reject) => {
        ...
        if (!cvs) return reject();

        let img = cvs.createImage();
        img.src = opt.src;
        img.width = opt.width;
  ...
  return (
    <View style={css.wrap}>
      <Canvas id={id} ref={refCVS} type='2d' style={css.cvs} />
      {/* <Canvas id={id} ref={refCVS} type='2d' style={css.cvs}>
        您的浏览器版本过旧
      </Canvas> */}
    </View>
